...
/**
 * Squeezes an array of time series data into fewer points by averaging values
 * @param data Array of data points with dates and amounts
 ...
 */
function squeezeTimeSeriesData(data: AccountTotalAmountProps[], targetPoints: number): AccountTotalAmountProps[] {
  if (!data?.length || targetPoints <= 0) return [];
  if (data.length <= targetPoints) return data;

  // Calculate chunk size (rounded up to ensure we don't exceed target points)
  const chunkSize = Math.ceil(data.length / targetPoints);

  // Split data into chunks
  const chunks: AccountTotalAmountProps[][] = [];
  ...
  return chunks.map(chunk => {
    const avgAmount = chunk.reduce((sum, item) => sum + item.accountAmount, 0) / chunk.length;

    // Use middle date from chunk as representative date
    const middleIndex = Math.floor(chunk.length / 2);
    const date = chunk[middleIndex].date;

    return {
      date,
      accountAmount: avgAmount
    };
  });
}
